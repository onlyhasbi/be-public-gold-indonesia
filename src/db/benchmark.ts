...
async function runBenchmark() {
  ...
  const actualAgentId = userRes.rows.length > 0 ? userRes.rows[0].user_id : "none";
  ...
  const explainLike = await db.execute({
    ...
    args: [actualAgentId, `%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`]
  });
  ...
  console.log(`Average TIME (LIKE): ${((endLike - startLike) / ITERATIONS).toFixed(2)} ms`);
  console.log(`Average TIME (FTS5): ${((endFts - startFts) / ITERATIONS).toFixed(2)} ms`);
